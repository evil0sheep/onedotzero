...
1. Clean up cluster.py
  1. usage message should print entire help message
  2. help message should include directions to get help on subcommands with e.g. `cluster compute --help` metrics
  3. `cluster compute down` should not log the ssh error since its expected given that we just killed the host were connected to
  4. make `cluster status` print last completed timestamps
  5. make a new command `cluster control cmd` which takes a command string and executes it on the appropriate directory on the remote host and once working remove `scripts/remote.py`. So for example `cluster control cmd "ls -la"` would return the directory listing of the directory that was just rsynced to the control node
  6. add a new command `cluster doc` which prints out a longform list of every command and a description of what it does.
  7. `cluster compute wait` should wait do 100 iterations with a 1 second wait interval instead of 15 iteration with a 10 second wait interval.
