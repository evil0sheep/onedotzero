...
1. Modify Ansible workflows to install vLLM and dependencies and verify proper configuration
  ...
  4. Make new ansible roles for Apache Ray that installs ray on both the compute nodes and the control node and configures Ray to use the control node as the Ray head node and the compute nodes as the Ray worker nodes. Verify in a new `verify.yml` that `ray status` shows the cluster topology that is expected given the definition of `compute_nodes` in `ansible/hardware_vars/0.1.yml`. Add the new `verify.yml` to `control_test.yml` (not hardware specific)
  ...
  6. add `compute-node` to `127.0.0.1` line in `/etc/hosts` on compute nodes
